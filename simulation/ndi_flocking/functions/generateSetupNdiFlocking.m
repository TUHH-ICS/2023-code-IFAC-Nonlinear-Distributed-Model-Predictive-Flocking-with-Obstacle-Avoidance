...
function setup = generateSetupNdiFlocking(cfg, param, init)
setup = struct;
...
for i = 1:length(Agents)
    Agents{i} = NdiFlockingAgent(Network.getId(), param, init.pos(:,i), init.vel(:,i), init.att(:,i), init.angVel(:,i), cfg);
end
setup.Agents = [Agents{:}];
end